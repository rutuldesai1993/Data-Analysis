...
public class Log {
	...
	private static final String USERNAME = "f17x321";
	private static final String PASSWORD = "f17x321TS";
	...
	public boolean createAccessLogs()
	{
		try
		{
			...
			String sqlQuery = "Create table if not exists f17x321.s18t22_log (LogID INT(6)" +
			...
			if (connect) 
			{
				...
				if (resultSet != null) 
				{
					...
					if (tableList.contains("s18t31_log")) 
					{
						String insertQuery = "Insert into f17x321.s18t22_log (Username,dbms," +
					...
					else
					{
						...
						{
							String insertQuery = "Insert into f17x321.s18t22_log " +
	...
	public int getMaxRowId() {
		int rowNumber = 0;
		try {
			String getRowId = "Select LogID from f17x321.s18t22_log where Username = '" + 
	...
	public boolean processLogout()
	{
		try
		{
			...
			{
				String updateQuery="Update f17x321.s18t22_log set LogoutTime= ? where " +
	...
	public String getLogData()
	{
		try {
			...
			{
				int userRows = 0;
				String selectQuery="Select count(*)  count from f17x321.s18t22_log where userName = '" + 
				...
				if(resultSet!=null)
				{
					...
					else
					{
						String sqlQuery = "select Username,dbms,LoginTime,LogoutTime,sessionID,ipAddress from " +
								"f17x321.s18t22_log where Username = '" + currentUserName + "' and LogID <> " +
