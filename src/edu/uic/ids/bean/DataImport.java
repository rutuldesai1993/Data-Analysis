...
public class DataImport {
	...
	public String processMetaDataFile(){
		...
		try {
			...
			if(computeVariable.size()>0){
				...
				sqlQuery = "create table "+bd.getSchema()+ ".f18t22_"+datasetLabel+" ("+nocomList.toString().replace("[", "").replace("]", "") +", "+comList.toString().replace("[", "").replace("]", "")  +" );";
				...
				sqlQuery = "create table "+bd.getSchema()+ ".f18t22_"+datasetLabel+" ("+SQLList.toString().replace("[", "").replace("]", "") + " );";
	...
	public String udado(){
		try{if (bd.getConnection() != null && bd.getStatement() != null) {
			sqlQuery = "truncate table "+bd.getSchema()+ ".s17g304_"+datasetLabel+";";
			bd.getStatement().executeUpdate(sqlQuery);
			...
			if(computeStatus == 0){
				if(format.equals("csv")){
					...
					for(int i =0; i<dataList.size(); i++){
						sqlQuery = "insert into "+bd.getSchema()+ ".f18t22_"+datasetLabel+ " values"+" ("+dataList.get(i).toString().replace("[", "").replace("]", "") + " );";
				...
				//Tab delimmited
					...
					for(int i =0; i<dataList.size()/variableList.size(); i++){
						...
						sqlQuery = "insert into "+bd.getSchema()+ ".s17g304_"+datasetLabel+ " values"+" ("+ab.toString().replace("[", "").replace("]","")+ " );";
			...
			//Computed
				...
				for(int j=0; j<dataList.size(); j++){
					...
					for(int i =0; i<b.size(); i++){
						sqlQuery = "insert into "+bd.getSchema()+ ".f18t22_"+datasetLabel+" ("+nocomputeVariable.toString().replace("[", "").replace("]", "")+" )" +" values"+" ("+b.get(i).toString().replace("[", "").replace("]", "") + " );";
				...
				//Tab delimmited
					...
					for(int i =0; i<dataList.size()/variableList.size(); i++){
						...
						sqlQuery = "insert into "+bd.getSchema()+ ".f18t22_"+datasetLabel+ " ("+nocomputeVariable.toString().replace("[", "").replace("]", "")+" )" +" values"+" ("+cd.toString().replace("[", "").replace("]", "")+ " );";
